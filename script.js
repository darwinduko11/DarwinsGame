...
const CONTENT = {
  regions: {
    crossroads: { id: "crossroads", name: "Crossroads of Veyr", scene: "scene-crossroads" },
    chapel: { id: "chapel", name: "Ashen Chapel", scene: "scene-chapel" },
    catacombs: { id: "catacombs", name: "Beneath the Chapel", scene: "scene-catacombs" },
    woods: { id: "woods", name: "Blackwood Forest", scene: "scene-woods" },
    inn: { id: "inn", name: "The Hollow Cup Inn", scene: "scene-inn" },
    marsh: { id: "marsh", name: "Marsh Edge", scene: "scene-woods" },
    tower: { id: "tower", name: "Veyr Bell Tower", scene: "scene-chapel" },
    southRoad: { id: "southRoad", name: "Ash Road", scene: "scene-crossroads" },
    dev: { id: "dev", name: "Developer Test Route", scene: "scene-crossroads" }
  },
  quests: {
    relic: {
      id: "relic",
      name: "Recover the Ashen Relic",
      regions: ["chapel", "catacombs"],
      endings: ["ending_relic", "ending_walkaway"],
      summary: "A chapel relic hidden beneath Veyr."
    },
    votive: {
      id: "votive",
      name: "The Votive Relic",
      regions: ["chapel", "catacombs", "tower"],
      endings: ["ending_mael", "ending_bond", "ending_break", "ending_corrupt", "ending_walkaway_adv"],
      summary: "A stolen votive relic, its fate unresolved."
    }
  },
  npcArcs: {
    Mael: { npc: "Mael", regions: ["chapel", "catacombs", "tower"], questIds: ["relic", "votive"] },
    Vessa: { npc: "Vessa", regions: ["marsh", "tower"], questIds: ["votive"] },
    Halven: { npc: "Halven", regions: ["woods", "chapel"] }
  },
  encounterTags: ["check", "fight", "sneak", "social", "exploration", "mystery", "reward", "ending"],
  endingTypes: ["restoration", "rupture", "corruption", "walkaway"]
};
...
const STORY = {
  ...
  mael_pay_success: {
    ...
    choices: [{ text: "Accept the warning and move on", next: "road_hub" }]
  },
  mael_pay_fail: {
    id: "mael_pay_fail",
    title: "Faith Unmoved",
    ...
    choices: [{ text: "Flee to the crossroads", consequences: { hp: -2, relation: { npc: "Mael", delta: -1 } }, next: "road_hub" }]
  },
  inn_hub: {
    id: "inn_hub",
  ...
  woods_lost: {
    ...
    choices: [{ text: "Escape the woods", consequences: { hp: -3 }, next: "road_hub" }]
  },
  ash_road: {
    id: "ash_road",
    title: "Ash Road",
  ...
  pilgrim_curse: {
    ...
    choices: [{ text: "Continue anyway", consequences: { hp: -2 }, next: "road_hub" }]
  },
  hidden_watch: {
    id: "hidden_watch",
  ...
  bandit_ambush: {
    ...
    choices: [
      ...
      { text: "Fight them off", check: { stat: "Strength", dc: 12, success: "bandits_defeated", fail: "bandits_hurt" } }
    ]
  },
  bandits_defeated: {
    id: "bandits_defeated",
    ...
    choices: [{ text: "Take their coin", consequences: { gold: 14, item: "Lockpicks", xp: 8 }, next: "road_hub" }]
  },
  bandits_hurt: {
    id: "bandits_hurt",
    ...
    choices: [{ text: "Stagger away", consequences: { hp: -4, gold: 6 }, next: "road_hub" }]
  },
  corpse_cart: {
    id: "corpse_cart",
    title: "Corpse Cart",
  ...
  cart_fail: {
    ...
    choices: [{ text: "Flee wounded", consequences: { hp: -5, gold: -3 }, next: "road_hub" }]
  },
  chapel_gate: {
    id: "chapel_gate",
  ...
  gate_fail: {
    ...
    choices: [{ text: "Retreat", consequences: { hp: -2 }, next: "road_hub" }]
  },
  catacombs_entry: {
    id: "catacombs_entry",
  ...
  catacombs_fail: {
    ...
    choices: [{ text: "Try again later", consequences: { hp: -2 }, next: "road_hub" }]
  },
  gate_open: {
    id: "gate_open",
...
const STORY_LINES = {
  world: [
    { id: "intro", location: "crossroads", chapter: "prologue", tags: ["mystery", "social"] },
    { id: "priest_meeting", location: "crossroads", chapter: "relic", tags: ["social", "quest"] },
    { id: "inn_hub", location: "inn", chapter: "rumors", tags: ["social", "exploration"] }
  ],
  arcs: [
    { id: "chapel", name: "Ashen Chapel", eventIds: ["chapel_gate", "chapel_inner", "catacombs_entry", "gate_open"] },
    { id: "mire", name: "The Mire", eventIds: ["mire_gate", "mire_shack", "mire_crossroads", "bell_tower"] },
    { id: "woods", name: "Blackwood", eventIds: ["woods_edge", "sir_halven", "woods_path", "shrine"] }
  ],
  hooks: [
    { id: "votive_relic", questId: "votive", eventIds: ["reliquary_open", "votive_decision", "return_mael", "ritual_choice"] },
    { id: "mael_relic", questId: "relic", eventIds: ["maels_request", "gate_open", "ending_relic", "ending_mael"] }
  ]
};
...
function addFlag(flag) { if (!hasFlag(flag)) state.flags.push(flag); }
function hasMemory(key) { return state.memories.some((m) => m.key === key); }
function addMemory(key, text) { if (!hasMemory(key)) { state.memories.push({ key, text }); notify(`Memory unlocked: ${text}`); } }
function notify(text) { state.notifications.unshift(text); state.notifications = state.notifications.slice(0, 8); }
function addLog(text) { state.log.unshift(text); state.log = state.log.slice(0, 25); }
...
function addItem(name) { if (!state.inventory.includes(name)) { state.inventory.push(name); notify(`Item gained: ${name}`); } }
function removeItem(name) { const idx = state.inventory.indexOf(name); if (idx >= 0) state.inventory.splice(idx, 1); }
function changeRelation(npc, delta) { if (!(npc in state.relationships)) state.relationships[npc] = 0; state.relationships[npc] += delta; notify(`${NPCS[npc]?.name || npc} relationship ${delta >= 0 ? "+" : ""}${delta}`); }
function changeQuest(q) { const existing = state.quests.find((x) => x.id === q.id); if (!existing) state.quests.push({ ...q }); else Object.assign(existing, q); notify(`Quest updated: ${q.name || q.id}`); }
function gainGold(amount) { state.gold = Math.max(0, state.gold + amount); notify(`${amount >= 0 ? "Gained" : "Lost"} ${Math.abs(amount)} gold`); }
function changeHP(amount) { state.hp = Math.max(0, Math.min(state.maxHp, state.hp + amount)); notify(`${amount >= 0 ? "Recovered" : "Lost"} ${Math.abs(amount)} HP`); }
...
function startNewGame() { state = defaultState(); saveGame(); render(); }
...
function resolveCheck(choice) {
  ...
  if (criticalSuccess && choice.check.critSuccess) {
    if (choice.check.critSuccessConsequences) applyConsequences(choice.check.critSuccessConsequences);
    showEvent(choice.check.critSuccess);
    addLog("Critical success.");
  } else if (criticalFailure && choice.check.critFailure) {
    if (choice.check.critFailureConsequences) applyConsequences(choice.check.critFailureConsequences);
...
el("saveBtn").onclick = () => { saveGame(); addLog("Game saved."); render(); };
el("loadBtn").onclick = () => { safeLoad(); };
